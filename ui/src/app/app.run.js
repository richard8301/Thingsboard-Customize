/*
 * Copyright © 2016-2018 The EMIXIoT Authors
 *
...
export default function AppRun(
  ...
  function initWatchers() {
    $rootScope.unauthenticatedHandle = $rootScope.$on(
      "unauthenticated",
      function(event, doLogout) {
        if (doLogout) {
          $state.go("login");
        } else {
    ...
    $rootScope.authenticatedHandle = $rootScope.$on(
      "authenticated",
      function() {
        UrlHandler($injector, $location);
      }
    ...
    $rootScope.stateChangeStartHandle = $rootScope.$on(
      "$stateChangeStart",
      function(evt, to, params) {
        function waitForUserLoaded() {
          ...
          $rootScope.userLoadedHandle = $rootScope.$on(
            "userLoaded",
            function() {
              $rootScope.userLoadedHandle();
        ...
        if (
          to.url === "/createPassword?activateToken" &&
          activateToken &&
          activateToken.length
        ) {
          userService.setUserFromJwtToken(null, null, false);
        }
          ...
          if (userService.isAuthenticated()) {
            ...
            if (userService.forceDefaultPlace(to, params)) {
              ...
              if (to.module === "public") {
                ...
                to.name === "home.dashboards.dashboard" &&
                $rootScope.forceFullscreen
              ) {
                evt.preventDefault();
    ...
    $rootScope.pageTitle = "EMIXIoT";

    $rootScope.stateChangeSuccessHandle = $rootScope.$on(
      "$stateChangeSuccess",
      function(evt, to, params) {
        ...
        if (angular.isDefined(to.data.pageTitle)) {
          $translate(to.data.pageTitle).then(
            function(translation) {
              $rootScope.pageTitle = "EMIXIoT | " + translation;
            },
            function(translationId) {
              $rootScope.pageTitle = "EMIXIoT | " + translationId;
            }
          );
        }
      }
    ...
    if (forbiddenDialog === null) {
      $translate([
        ...
        if (forbiddenDialog === null) {
          ...
          $mdDialog.show(forbiddenDialog).then(
            function() {
